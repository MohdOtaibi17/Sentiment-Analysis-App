...
if 'df' in locals() and df is not None and "review" in df.columns:
    ...
    # Average positivity
    avg_positive = round((sentiment_counts.get("Positive 😀", 0) / len(df)) * 100, 2)
    st.success(f"✅ Percentage of happy customers: {avg_positive}%")

    # Top positive & negative reviews
    st.subheader("✨ Highlighted Reviews")
    pos_reviews = df[df["Sentiment"] == "Positive 😀"]["review"].head(3).tolist()
    neg_reviews = df[df["Sentiment"] == "Negative 😡"]["review"].head(3).tolist()
    ...
    st.info("👈 Please upload a CSV file or enter text manually to start analysis.")
